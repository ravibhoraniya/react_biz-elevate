...
class AdsetsComponent extends Component {
  ...
  handleGenderChange = gender => {
    ...
    if (gender.id === 0) {
      ...
      userData.data.targeting.genders.push(gender.id);
    }
  };
  handleMinAge = minAge => {
  ...
  handleTargetingSearch = data => {
    ...
    if (userData.data.targeting[data.type]) {
  ...
  render() {
    const { selectedLocation, selectedLanguages, targetingData, errors } = this.state;
    if(this.props.adsets && this.props.adsets.success){
      this.props.handleClick();
    }
    return (
        <div id="right-panel" className="adsets right-panel">
